...
def get_parser():
    ...
    parser.add_argument("--transfer", default=None, type=str, choices=["ppgbp", "sensors", "uci2", "bcg", "mimic_ecg", "vital_ecg"])
    parser.add_argument("--target", default=None, type=str, choices=["ppgbp", "sensors", "uci2", "bcg", "mimic_ecg", "vital_ecg"])
    ...
    parser.add_argument("--baseline", default="ft", type=str, choices=["ft", "lp"])
    ...
    parser.add_argument("--wandb_project_name", type=str, default='your_project_name')
...
def main(args):
    ...
    config.param_trainer.max_epochs=config.epochs 
    config.param_trainer.check_val_every_n_epoch=2
    config.param_model.batch_size=args.batch_size
    config.param_early_stop.patience=config.num_patience 
    config.exp.N_fold=5
...
if __name__ == '__main__':
    ...
    elif args.baseline == 'ft':
        group_name = group_name + '-finetuning'
    if args.method == 'prompt_global':
        group_name = group_name + f'-prompt_global'
    
    if not args.ignore_wandb:
        import wandb
        wandb.init(entity='your_entity', project=f"{args.wandb_project_name}", group=group_name)
        lr = args.lr
        wd = args.wd
